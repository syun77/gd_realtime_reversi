...
# 三次関数
static func cube_in(t:float) -> float:
	return t * t * t
static func cube_out(t:float) -> float:
	return 1 + (t - 1) * (t - 1) * (t - 1)
static func cube_in_out(t:float) -> float:
	if t <= 0.5:
		return t * t * t * 4
	else :
...
# スムーズ曲線
static func smooth_step_in(t:float) -> float:
	return 2 * smooth_step_in_out(t / 2)
static func smooth_step_out(t:float) -> float:
	return 2 * smooth_step_in_out(t / 2 + 0.5) - 1
static func smooth_step_in_out(t:float) -> float:
	return t * t * (t * -2 + 3)
	
# よりスムーズな曲線
static func smoother_step_in(t:float) -> float:
	return 2 * smoother_step_in_out(t / 2)
static func smoother_step_out(t:float) -> float:
	return 2 * smoother_step_in_out(t / 2 + 0.5) - 1
static func smoother_step_in_out(t:float) -> float:
...
static func exec(type:eType, t:float) -> float:
	var function = get_function(type)
	return function.call(t)

# @param v 0.0〜1.0
static func step(type:eType, start:float, end:float, v:float) -> float:
	if start == end:
	...
	return a + (d * exec(type, t))
	

static func get_function(type:eType) -> String:
	var tbl = {
